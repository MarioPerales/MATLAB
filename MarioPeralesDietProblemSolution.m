%%% On output: Gives solution to Diet Problem


%%% We want to solve the diet problem. Table A is found here. (We just want
%%% the nutrition intake

data=[[36.0, 12600.0, 44.7, 1411.0, 2.0, 365.0, 0.0, 55.4, 33.3, 441.0, 0.0];
 ...
 %%% b is given by Table 1. It is exactly those entries.
  b = [3.0, 70.0, 0.8, 12.0, 5.0, 1.8, 2.7, 18.0, 75.0]';
 %%% Recall that the diet problem is trying to satisfy the daily
 %%% minimum requirements imposed by a diet (i.e. b). Therefore we are
 %%% concerned with the data that has to do about nutritional value of 
 %%% an item. These are the items in columns 3 through 11.
  A = data(1:end,3:end)'; 
 %%% We take the inverse because we want to have a system of equations that
 %%% reflects (nutritional value of item 1) + ... + (nutritional value of 
 %%% item 77) >= (daily minimum requirement for the nutriton).
 %%% NOTE: We have a ">=" sign, thus we must introduce slack variables.
 A = [A -eye(9)];

 %%% We now have the CMP: [A -e][X] = b; [x, z] >= 0; sum(x) = min;
 %%%                            [z]

 c = [ones(77,1); zeros(9,1)];
  
[x, obj] = MarioPeralesLPSolver(A,b,c);
disp('Our optimal solution is');
disp(x);
disp('Our optimal objective value (i.e. daily cost) is...');
disp(strcat(num2str(obj), ' $'));
disp('Therefore our annual cost to fulfill the diet is...');
str = num2str(obj*365);
disp(strcat(str, ' $'));
